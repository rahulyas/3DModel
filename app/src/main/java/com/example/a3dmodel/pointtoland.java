...
import com.example.a3dmodel.myView.NewActivity;
import com.example.a3dmodel.myView.firstActivity3D;
...
public class pointtoland extends AppCompatActivity {
    ...
    List<Integer> facepointslist = new ArrayList<>();

    static HashSet<Triangle> triangulation1;

    public static ArrayList<Double> facepointslist1 =new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        nextactivity.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(pointtoland.this, NewActivity.class);
...
/*                    new Utils().readText(new Utils().getFilePath(fileuri),this);
                    new_finallist = new Utils().readNorthingEasting();*/
                    list = new Utils().readAnyfile(fileuri, this);
...
//            pointlist.add(p.toString());
        ...
        double northingmax = Collections.max(FirstNorthing);
        double northingmin = Collections.min(FirstNorthing);

        double eastingmax = Collections.max(FirstEasting);
        double eastingmin = Collections.min(FirstEasting);

        double elevationmax = Collections.max(FirstElevation);
        double elevationmin = Collections.min(FirstElevation);
        ...
        for(int q=0;q<FirstNorthing.size();q++){
            Point p = new Point(FirstNorthing.get(q), FirstEasting.get(q), FirstElevation.get(q), id++);
            pointList2.add(p);
        }
...
//        Log.d(TAG, "pointlist:=" + pointlist);
//        triangulation1 = getTriangulation(pointList);
        triangulation1 = getTriangulation(pointList2);
        Log.d(TAG, "pointList=triangulation:=" + triangulation1);
        Log.d(TAG, "pointList=triangulation:Size=" + triangulation1.size());
//        makeOBJ(pointlist);

        for (Triangle triangle : triangulation1) {
    ...
    /*        Log.d(TAG, "pointList=point1:=" + point1 +"=="+x+"=="+y+"=="+z);
        ...
        for(int k=0 ;k< NorthingintegerList.size(); k++){
            facepointslist.add(NorthingintegerList.get(k));
            facepointslist.add(EastingintegerList.get(k));
            facepointslist.add(ElevationintegerList.get(k));
        }

        for(int j=0;j<facepointslist.size();j++){
            int value = facepointslist.get(j);
            System.out.println("value == "+value);
            facepointslist1.add(FirstNorthing.get(value-1));
            facepointslist1.add(FirstEasting.get(value-1));
            facepointslist1.add(FirstElevation.get(value-1));
        }
        System.out.println("facepointslist =="+facepointslist);
        System.out.println("facepointslist1 =="+facepointslist1);
